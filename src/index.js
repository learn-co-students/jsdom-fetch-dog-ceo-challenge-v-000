...
function getDogBreeds() {
    ...
    .then((json) => {
        for (const key in json.message) {
            ...
            // solution adds an event listener to the li after the append to the parent
            li.addEventListener('click',function(event) {
                event.target.style.color = "orange";
            })
        }
    })
    // add the event listener to the dropdown when loading up the page so it's available
    filterBreed();
}
